const reportLocale = {
  ...
  releaseNotes: {
    "England's bronze, live freshness, and smoother mobile controls": {
      title: "잉글랜드의 동메달, 실시간 업데이트, 더 부드러운 모바일 컨트롤",
      highlights: [
        "잉글랜드가 프랑스를 6-4로 꺾고 3위를 차지한 경기에 공식 점수, 전체 득점 타임라인과 라인업, 출처가 있는 세 줄 요약, FOX Sports 하이라이트가 추가되었습니다.",
        "모바일에서 Ball Boy와 토너먼트의 다음 경기 보기 버튼은 이제 순서가 정해진 전환을 사용합니다. Ball Boy가 먼저 자리를 비운 뒤 버튼이 나타나고, 버튼이 먼저 사라진 뒤 Ball Boy가 내려오며, 패널 중단이나 실시간 재렌더링에도 두 요소가 겹치지 않습니다. 데스크톱 동작은 그대로입니다.",
        "바닥글은 이제 출처 보기, 최신 데이터 업데이트 시간, 릴리스 노트 보기 링크만 간결하게 표시하며 비공식 예측 안내를 반복하지 않습니다.",
        "데이터 업데이트가 오늘이면 날짜 표현을 생략하고, 어제이면 ‘어제’를 유지하며, 그보다 오래된 경우 날짜를 표시합니다. 날짜와 시간은 선택한 시간대를 따르며 시간대 약어를 반복하지 않습니다.",
        "메인 앱과 오류 제보 페이지는 이제 실시간 데이터 확인이 성공하면 해당 시간을 업데이트 기준으로 사용하고, 필요할 때만 정적 경기 시간으로 대체하며, 동일한 문구와 선택한 시간대 동작을 공유합니다.",
        "자동 검사는 이제 모바일 스크롤 반전, 실시간 버튼 교체, 패널 중단, 가로 화면, 동작 줄이기, 실시간 업데이트 성공과 대체 경로, 로스앤젤레스와 도쿄 시간대를 다룹니다."
      ]
    },
