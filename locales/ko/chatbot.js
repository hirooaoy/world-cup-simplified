...
const copy = {
  ...
  suggestions: ["오프사이드를 설명해 줘", "누가 우승했어?", "아르헨티나는 어떻게 뛰어?", "오류 제보"],
  ...
  worldCupContext: (years) => `${years} 월드컵 당시`,
  ...
  fullRecord: "대회 전체 성적",
  tournamentFinish: "대회 최종 성적",
  tournamentFinal: "결승",
  tournamentAwards: "대회 수상",
  tournamentMatches: "경기",
  tournamentGoals: "득점",
...
const intents = {
  ...
  replacements: [
    [/월드컵 수상자|월드컵 수상 결과|월드컵 상/g, " world cup awards "],
    [/골든볼/g, " golden ball "], [/골든부트/g, " golden boot "], [/골든글러브/g, " golden glove "],
    [/영플레이어|최우수 신인/g, " young player "], [/페어플레이/g, " fair play "],
    [/월드컵 요약|월드컵을 요약|월드컵 정리/g, " summarize world cup "],
    [/다음 월드컵|2030 월드컵/g, " next world cup "],
    [/어디까지 갔|몇 강까지 갔|어디에서 탈락/g, " how far did go "],
    [/월드컵 최다 우승국|가장 많이 우승한 나라/g, " who won most world cups "],
    [/이전 월드컵/g, " previous world cup "],
    [/지난 월드컵|저번 월드컵|지난번/g, " last world cup "],
...
const templates = {
  ...
  h2hNone: () => "이 출처에서 이전 맞대결이 반환되지 않았습니다. 전체 역사가 완전한지는 확인되지 않았습니다.",
  h2hRecord: (d) => `${d.coverageStatus === "complete" ? "확인된" : "데이터셋에서 확인할 수 있는"} 성인 국가대표 맞대결 ${d.total}경기에서 ${d.first} ${d.firstWins}승, 무승부 ${d.draws}회, ${d.second} ${d.secondWins}승을 기록했고 총 ${d.goals}골이 나왔습니다.${d.coverageStatus === "complete" ? "" : " 전체 역사가 완전한지는 확인되지 않았습니다."}`,
