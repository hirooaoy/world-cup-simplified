...
For unresolved future knockout rounds, never synthesize 1X2 pills from FIFA rankings. If credible lookahead markets exist for multiple possible matchups, store them as `conditionalProjections` on the future fixture. Each entry must identify the exact team pair, use `market: "match-winner"`, total 100 across `home` and `away`, preserve at least two normalized source inputs, and be captured before kickoff. The Tournament view renders these as two-way chances to win the match including extra time and penalties; it must not invent a regulation-time draw price. If no sourced conditional forecast exists for the projected pair, show no percentage until the matchup is confirmed and direct 1X2 markets can be loaded.

Conditional consensus should blend matchup-relevant Opta tournament simulations with current bookmaker or prediction-market prices. Normalize each source to a two-way 100% market, then use the rounded unweighted mean of the preserved source inputs for the displayed consensus. Record the derivation for every input so validation and an audit can reproduce the result. Once both participants are confirmed, replace the conditional forecast with the normal stored `projection` from direct regulation-time markets; the standard three outcome pills then return automatically.
