...
After changing the archived Best XI selections, run `pnpm profiles:historical:best-xi:enrich`. It adds sourced birth dates and tournament evidence for all 484 starter and honourable-mention cards. For market value, it records the last source-backed Transfermarkt valuation on or before that edition's opening date plus a sourced career peak; when the historical dataset has no defensible valuation, it stores an explicit unavailable reason instead of estimating one. Run `pnpm audit:historical:best-xi` to verify age, evidence, two-paragraph copy in all four locales, and complete value provenance.
...
The complete 2026 edition is preserved in `data/fixtures.json`: all 104 fixtures have final participants, scores, stages, and venues, with final standings, official line-ups, multilingual recaps, awards, player profiles, and reviewed provenance. The featured edition is now static and does not require matchday polling or placeholder-team maintenance.

Future official corrections must use the reviewed late-correction workflow described below. Ordinary UI, accessibility, or cache maintenance does not change tournament history and must not create a correction snapshot.

## Edition lifecycle and the 2026 archive

`data/edition-lifecycle.json` is the explicit, edition-neutral guard for live-era work. It owns the featured edition, timezone, expected tournament shape, provider identifiers, and a half-open live-sync window (`tournamentStartsAt <= now < liveSyncEndsAt`). The browser and `/api/live-data` read the same boundary and fail closed: outside an active `live` window they use static edition data and do not contact upstream providers. Ordinary push and pull-request validation remains available after closure. Matchday workflows have no recurring schedule while the edition is archived; their manual dispatch defaults to a read-only preview and requires explicit confirmation plus a reviewed pull request for a live update or late official correction. Run `pnpm edition:window:smoke`, `pnpm smoke:archived-runtime`, and `pnpm audit:workflows` after changing lifecycle behavior.
...
Run `pnpm archive:2026:smoke` to verify the finalizer itself. After all 104 fixtures, official final lineups, four-language result stories, highlight dispositions, final standings, and the confirmed Golden Ball, Golden Boot, Golden Glove, Young Player, and Fair Play awards are complete—and the archive eligibility time has passed—run `pnpm archive:2026:plan`. This writes a durable review plan under `data/archive-plans/` containing the exact archive timestamp, version, input hashes, output hashes, and immutable snapshot SHA-256.

Review that plan and run `pnpm sanity:2026` without changing archive inputs. Then consume the exact reviewed plan with `pnpm archive:2026 -- --plan-file data/archive-plans/<plan-file>.json`. The writer refuses an absent, stale, or altered plan. It atomically writes a timestamped immutable snapshot plus `data/archives/world-cup-2026-manifest.json`, records the exact archive file and checksum in the lifecycle, and marks the lifecycle archived only after the other files commit. The snapshot preserves fixtures, teams, standings, multilingual result evidence, official lineups, pre-match prediction history, player availability, player and coach profiles, Ball Boy match knowledge, team-style profiles, canonical awards, corrections, tournament metadata, and exact input provenance. Schema 4 also preserves the 2026 editorial Best XI dataset: 11 starters, 15 displayed Honorable Mentions across the 11 roles, all 23 researched alternatives, Best Coach, four-language rationale copy, and the canonical profile metadata used by its cards.

Immediately run `pnpm archive:verify`. This independently follows lifecycle to the manifest and every immutable snapshot, recomputes each SHA-256 checksum, validates the edition, archive chain, provenance, and core record counts, and refuses mutable edition data that no longer matches the latest snapshot. The monthly Data Quality schedule repeats this read-only archive health check; push and pull-request CI continues after closure.

The archive also records SHA-256 evidence for the exact site surface shipped at cutover. Run `pnpm archive:verify:surface` during this release to prove those bytes still match. Normal ongoing `pnpm archive:verify` deliberately does not require future UI, accessibility, or cache-only maintenance to retain the 2026 shell hashes; only tournament-data changes require a late-correction archive.

`history.json` deliberately remains the 1930–2022 legacy archive while `fixtures.json` remains the rich 2026 edition dataset, even after 2026 is archived. Adding the same matches to both sources would duplicate calendar and search results, while forcing 2026 through the older historical renderer would discard richer lineups, profiles, awards, and match presentation. Before 2030, introduce an additive edition registry and compatibility adapter so archived status and presentation capability are separate concerns; do not append 2026 directly to `history.json`.

For a later official correction, first prepare and review the data through a workflow's `late-correction` mode. Then create a new plan with `pnpm archive:2026:plan -- --late-correction`, review it, and consume it with `pnpm archive:2026 -- --plan-file data/archive-plans/<plan-file>.json --late-correction`. Each correction creates a new immutable snapshot that points to the version it supersedes; prior archive files are never overwritten.
